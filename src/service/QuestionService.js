...
class QuestionService{
    ...
    createQuestion(email,question){
        return axios.post(QUESTION_BASE_REST_API(id)+`?email=${email}`,question,authHeader())
    }
    getQuetionById(questId){
        return axios.get(QUESTION_BASE_REST_API(id)+`/${questId}`,authHeader())
    }
    updateQuestion(questId,question){
        return axios.patch(QUESTION_BASE_REST_API(id)+`/${questId}`,question,authHeader())
    }
